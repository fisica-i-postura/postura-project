...
class PhysicsProcessor:
    ...
    def _process_video(self) -> tuple[VideoMetadata, pd.DataFrame]:
        video_input = VideoInput(self.user_input.video_path, 12, 14, self.user_input.shoulder_elbow_distance_in_meters)
        video_output = VideoTracker(video_input).process()
        video_metadata = VideoMetadata(video_input.path, video_output.fps, video_output.resolution, self.user_input.subject_gender.value, self.user_input.subject_weight, self.user_input.shoulder_wrist_distance_in_meters, video_output.pixels_per_meter, video_output.base_line_offset_in_px)
